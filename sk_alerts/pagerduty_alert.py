...
class PagerdutyAlert:
	...
	"""
		self.SERVICE_KEY = option['service_key']
		self.API_KEY_TOKEN = option.get('api_key_token', None)
	...
	def trigger_incident(self, payload )
		...
		request_payload = json.dumps( build_payload("trigger", payload))
	...
	def resolve_incident(self, payload):
		...
		"""
		payload = json.dumps( build_payload("resolve", payload) )
		response = requests.post(
				'https://events.pagerduty.com/generic/2010-04-15/create_event.json',
...
pdAlert = PagerdutyAlert(service_key="myservicekey")
